...
public class RichWPSClientSession {
	...
	private static Logger LOGGER = LoggerFactory
			.getLogger(RichWPSClientSession.class);
	 ...
	 */
	public boolean connect(String wpsurl, String richwpsurl)
			throws WPSClientException {
		LOGGER.info("CONNECT");
	 ...
	 * @param doc
	 *            DeployProcess document
	 * @return a DeployProcessResponseDocument
	 */
	public Object deploy(String serverId, DeployProcessDocument doc)
			throws WPSClientException {
		String richwpsurl;
		if (loggedTransactionalServices.containsKey(serverId)) {
			richwpsurl = loggedTransactionalServices.get(serverId);
			LOGGER.info("DeployProcess: "
					+ doc.getDeployProcess().getProcessDescription()
	...
	/**
     */
	public Object undeploy(String serverId, UndeployProcessDocument doc)
			throws WPSClientException {
		String richwpsurl;
		if (loggedTransactionalServices.containsKey(serverId)) {
			richwpsurl = loggedTransactionalServices.get(serverId);
			LOGGER.info("UndeployProcess: "
					+ doc.getUndeployProcess().getProcess().getIdentifier()
							.getStringValue());
		...
		if (loggedTransactionalServices.containsKey(serverId)) {
			...
			LOGGER.info("TestProcess: "
					+ doc.getTestProcess().getProcessDescription()
							.getIdentifier().getStringValue());
			return this.retrieveTestProcessResponseViaPOST(richwpsurl, doc);
		} else {
			LOGGER.info("Server ID " + serverId
					+ " not registered as RichWPS service");
			return null;
	...
	public Object getSupportedTypes(String serverId,
			GetSupportedTypesDocument doc) throws WPSClientException {
		String richwpsurl;
		if (loggedTransactionalServices.containsKey(serverId)) {
			richwpsurl = loggedTransactionalServices.get(serverId);
			LOGGER.info("GetSupportedTypes");
			return this.retrieveGetSupportedTypesResponseViaPOST(richwpsurl,
					doc);
		} else {
			LOGGER.info("Server ID " + serverId
					+ " not registered as RichWPS service");
	 ...
	 * @throws WPSClientException
	 */
	private Object retrieveDeployProcessResponseViaPOST(String url,
			DeployProcessDocument doc) throws WPSClientException {
		...
		try {
			...
			try {
				...
				throw new WPSClientException(
						"Error occured while parsing deployProcessResponse", e);
			}
		}
	}
	 ...
	 * @throws WPSClientException
	 */
	private Object retrieveUndeployProcessResponseViaPOST(String url,
		...
		try {
			...
			try {
				...
				throw new WPSClientException(
						"Error occured while parsing undeployProcessResponse",
						e);
			}
		}
	}
	...
	/**
	 * Retrieves a TestProcess response via Http POST request and tries to parse
	 * it
	 *
	 * @param url
	 * @param doc
	 * @return either an TestProcessResponseDocument or an
	 *         ExceptionReportDocument
	 * @throws WPSClientException
	 */
	private Object retrieveTestProcessResponseViaPOST(String url,
			TestProcessDocument doc) throws WPSClientException {
		InputStream is = retrieveDataViaPOST(doc, url);
		Document documentObj = checkInputStream(is);
		try {
			return TestProcessResponseDocument.Factory.parse(documentObj);
		} catch (XmlException e) {
			try {
				return ExceptionReportDocument.Factory.parse(documentObj);
			} catch (Exception e2) {
	 ...
	 * @throws WPSClientException
	 */
	private Object retrieveGetSupportedTypesResponseViaPOST(String url,
			GetSupportedTypesDocument doc) throws WPSClientException {
		InputStream is = retrieveDataViaPOST(doc, url);
		Document documentObj = checkInputStream(is);
		try {
			...
			try {
				...
				throw new WPSClientException(
						"Error occured while parsing SupportedTypesResponseDocument",
						e);
			}
		}
	}
