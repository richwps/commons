...
public class RichWPSLocalClientExample {
	...
	public void testGetSupportedTypes() throws WPSClientException {
		...
		Object responseObject = wpstClient.getSupportedTypes(wpsURL,
				builder.build());
	...
	public void testDeploy() throws WPSClientException {
		...
		ProcessDescriptionType.ProcessOutputs outputs = ProcessDescriptionType.ProcessOutputs.Factory
				.newInstance();
		ProcessDescriptionTypeBuilder bpdt = new ProcessDescriptionTypeBuilder(
				"test", "test", "1", outputs);
		...
		// deploy
		Object responseObject = wpstClient.deploy(wpsURL,
	...
	public void testUndeploy() throws WPSClientException {
		...
		// undeploy
		Object responseObject = wpstClient.undeploy(wpsURL,
				builder.getUndeploydocument());
	...
	public void listProcesses() {
		...
		try {
			...
			ProcessDescriptionsDocument pdd = this.wpsClient
					.describeAllProcesses(wpsURL);
			ProcessDescriptionsDocument.ProcessDescriptions descriptions = pdd
					.getProcessDescriptions();
			ProcessBriefType[] descs = descriptions
			...
			for (ProcessBriefType process : descs) {
				if (process.getIdentifier() != null) {
					...
					System.err.println("Debug:getAvailableProcesses()"
							+ process);
				}
			}
		} catch (WPSClientException e) {
			System.err
					.println("Debug:getAvailableProcesses()#WPSClientException:\n "
							+ e);
		} catch (Exception e) {
	...
	public static void main(String[] args) throws WPSClientException {
		...
		String wpstURL = "http://localhost:8080/wps/RichWPS";

		RichWPSLocalClientExample client = new RichWPSLocalClientExample(
				wpsURL, wpstURL);
